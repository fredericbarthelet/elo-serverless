...
import type { PutItemCommandInput, TransactWriteItemsInput, TransactGetItemsCommandInput } from "@aws-sdk/client-dynamodb";
...
const transaction: TransactWriteItemsInput = {
    TransactItems: [addPlayerA, addPlayerB, addGameResult].map(putItemOperation => ({ Put: putItemOperation}))
}

const getPlayerElos: TransactGetItemsCommandInput = {
    TransactItems: [playerAPK, playerBPK].map(playerKey => ({ Get: { Key: marshall({ PK: playerKey }), TableName: stackUnderTest.table.tableName }}))
}

test.assertions.awsApiCall('DynamoDB', 'transactWriteItems', transaction).provider.addToRolePolicy({
    Effect: 'Allow',
    Action: ['dynamodb:PutItem'],
    Resource: ['*'],
})
test.assertions.awsApiCall('DynamoDB', 'transactGetItems', getPlayerElos).provider.addToRolePolicy({
    Effect: 'Allow',
